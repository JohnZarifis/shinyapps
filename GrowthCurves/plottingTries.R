...
SamplingTsipoyra <- Sampling[Sampling$SPECIE %in% c("ΛΑΒΡΑΚΙ") 
                             & Sampling$GRADING == 'Α' & Sampling$AvgWeight <= 420 & Sampling$ORIGINMONTH %in% c(1,2,3),]
...
m0A <- gamlss(AvgWeight~pb(Tage)
              ,sigma.formula =~pb(Tage)
              ,nu.formula = ~pb(Tage)
...
centiles(m0A, xvar=keep$Age,  main="Abdominal data \n @")

round(Q.stats(m0, xvar=keep$Age),3)
wp(m0, xvar=keep$Age, n.inter=9)
fittedPlot(m0, x=keep$Age)

a<- gamlss(AvgWeight~pb(Age) 
           , sigma.fo=~pb(Age)
           , data = keep
           , trace = FALSE 
           , nu.formula = ~pb(Age)
...
a<- gamlss(AvgWeight~lo(~Age,span=0.4),data = keep, family = NO)

centiles(a, xvar=keep$Age,  main="Abdominal data \n @")
...
###### just plotting 
keepM <- keep %>% group_by(LOT,GRADING) %>%
  summarise(
    l = min(GPD),
    m = mean(GPD),
    u = max(GPD)
  ) %>% mutate(LOT = reorder(LOT,m,function(x)x))


ggplot(data = keepM ,aes(x=LOT , y = m ,ymin = l, ymax = u  )) + geom_pointrange(aes(colour = GRADING)) +
  coord_flip()


ggplot(data = keep, aes(x = as.factor(ORIGINMONTH),y=GPD)) + geom_boxplot()
